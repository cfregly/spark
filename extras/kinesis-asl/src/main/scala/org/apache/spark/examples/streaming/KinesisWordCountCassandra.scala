 ...
 *
 * Usage: KinesisWordCountCassandra <stream-name> <endpoint-url>
 ...
 *        org.apache.spark.examples.streaming.KinesisWordCountCassandra mySparkStream \
 *        https://kinesis.us-east-1.amazonaws.com
 */
object KinesisWordCountCassandra extends Logging {
...
//    val numStreams = numShards
    ...
    def createStreamingContext(): StreamingContext = {
      val sparkConfig = new SparkConf().setAppName("KinesisWordCountCassandra")
...
//    val collection = ssc.parallelize(Seq(("XX1", "XY1", 0), ("XX1", "XY2", 1)))
...
//    
    ...
    val unionStreams = KinesisUtils.createStream(ssc, streamName, endpointUrl, kinesisCheckpointInterval,
        InitialPositionInStream.LATEST, StorageLevel.MEMORY_AND_DISK_2)
...
//    wordsDStream.count().foreachRDD(countRDD => {globalRDDCount += countRDD.first()})
    ...
    wordCountsMappedDStream.saveToCassandra("sad", "activity", SomeColumns("from", "to"))
...
//      rdd.foreach(row => println((row._1, row._2)
//          //,  globalBatchCount, globalRDDCount, globalSum
//          )))
//    })
...
//      rddBatch.foreach(row => println((row._1, row._2)
//          //, globalBatchCount, globalRDDCount, globalSum
//          )))
//    })
...
/**
 * Usage: KinesisWordCountCassandraProducer <stream-name> <kinesis-endpoint-url>
 ...
 *    $ export AWS_SECRET_KEY=<your-secret-key>
 *    $ $SPARK_HOME/bin/run-example \
 *         org.apache.spark.examples.streaming.KinesisWordCountCassandraProducer mySparkStream \
 *         https://kinesis.us-east-1.amazonaws.com 10 5
 */
object KinesisWordCountCassandraProducer {
  def main(args: Array[String]) {
    if (args.length < 4) {
      System.err.println("Usage: KinesisWordCountCassandraProducer <stream-name> <endpoint-url>" +
